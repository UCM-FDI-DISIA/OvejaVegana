...
OvejaVegana::SmokeComponent::SmokeComponent() { }

std::pair<bool, std::string> OvejaVegana::SmokeComponent::InitComponent() {
    return { true, "SmokeComponent created correctly" };
}

void OvejaVegana::SmokeComponent::Update(const double& dt) {
    duration -= dt;
...
void OvejaVegana::SmokeComponent::CreateSmokeEffect(const VeryReal::Vector3& position, int numB) {
